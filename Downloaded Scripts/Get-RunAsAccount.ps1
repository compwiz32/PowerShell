function Get-RunAsAccount {
<#
.SYNOPSIS
Get RunAsAccount for services and scheduled tasks on the specified computer using Windows Management Instrumentation (WMI) and schtasks.exe
...
.EXAMPLE
Get-RunAsAccount -ComputerName srv01 -RunAsUser contoso

.EXAMPLE
Get-RunAsAccount -ComputerName (Get-Content c:\computernames.txt)
