<# 
  .Synopsis 
  Exports information about Servers and the Domain Controllers at individual sites.

  .Description 
  Will gather information on all servers and domain controllers at 1 specified site at a time. When 
  the script is run, a menu option will appear to the user and they can select which site they would 
  like the server information from. This list has all of the sites globally and it is separated into 
  region to ease of use. The information is exported to a preformatted HTML page. The formatting of 
  the HTML page has alternating row colors which facilitates reading quickly. Will ask the user where 
  they would like the file saved at the end of the script.
  This script was written to pull a report for Mike Kanakos.

  .Example 
  Get-ServersIndividualSitesIncludeDC
  Asks the user to input a filepath to save the CSV to and then returns a variety of information on servers
  and domain controllers at the chosen site.
