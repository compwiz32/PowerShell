...
function Test-OneDriveIllegalCharacters
{
  <#
      .SYNOPSIS
      ...
      Returns true/false if a string contains illegal characters.
      .EXAMPLE
      Test-OneDriveIllegalCharacters -string 'sdofijs'
      False
      .EXAMPLE
      Test-OneDriveIllegalCharacters -string '#%*:<>'
      True
  #>
  [CmdletBinding()]
  param
...
function Resolve-OneDriveIllegalCharacters
{
  <#
      .SYNOPSIS
      ...
      Removes illegal characters from a string and returns a 'corrected' value [str].
      .EXAMPLE
      Resolve-OneDriveIllegalCharacters -string 'bad_file#ame%!<<>'
...
function Repair-OneDriveNamingIssues
{
  <#
      .SYNOPSIS
      ...
      C:\working\f2lj#%35#$\2348^^%%\no$^no@^
      .EXAMPLE
      Repair-OneDriveNamingIssues -FolderPath c:\working -TestRun
  ...
  foreach ($f in ($AllFolders | Sort-Object -Property FullName -Descending))
  {
    ...
    if (Test-ODfBIllegalCharacters -string $f.BaseName)
    {
      ...
      Write-Host -Object "$($f.FullName)" -ForegroundColor Red
            Write-Host -Object "$($f.FullName.Replace(($f.BaseName),(Resolve-OneDriveIllegalCharacters -string $f.BaseName)))" -ForegroundColor Cyan

            $newname = "$($f.FullName.Replace(($f.BaseName),(Resolve-OneDriveIllegalCharacters -string $f.BaseName)))"
      if (!$TestRun)
      {
  ...
  foreach ($f in ($AllFiles | Sort-Object -Property FullName -Descending))
  {
    ...
    #Test that this folder has no illegal characters.
        if (Test-OneDriveIllegalCharacters -string $f.BaseName)
    {
      Write-Host -Object 'Found Illegal Name: ' -NoNewline
      Write-Host -Object "$($f.FullName)" -ForegroundColor Red
