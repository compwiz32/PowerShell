...
foreach($Computer in $ComputerName) {
    ...
    if(Test-Connection -ComputerName $Computer -Count 1 -Quiet) {
        Write-Verbose "$Computer : Online" -Verbose
        try {
            ...
            Write-Verbose "Successfully added $ObjectName $ObjectType to $Computer" -Verbose
        } catch {
            $Status = "Failed"
            $Comment = $_.toString().replace("`n","").replace("`r","")
            Write-Verbose "Failed to add $ObjectName $ObjectType to $Computer" -Verbose
        }
